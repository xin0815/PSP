...
<html>
	<head>
		...
		$(function(){
			$("#fhBtn").click(function(){
		        window.location.href='<%=basePath%>classStudent/list';
			});
		});
			function upload(){
	...
	<body>
		...
		<form action="classStudent/importStudent" method="post" enctype="multipart/form-data" id="uploadForm">
			<input type="hidden" name="cid" value="${bc.cid }" /><br/>
			请单击此处，下载导入模板：<a href="<%=basePath%>template/studentInfo.xls" class="btn btn-info active" role="button">分班后学生信息导入表模版下载</a>
			<br/>
			<br/>
			<div class="well form-search">
				<h5>班级名称：${bc.cclassName }</h5>
				<br />
				<div class="a-upload">
				...
				<button type="button" class="btn btn-info" id="fhBtn">
   					返回
   				</button>
			</div>
