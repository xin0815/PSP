...
dhtmlXTreeObject.prototype._serializeItem = function(itemNode) {
	...
	if (!this._xfullXML)
		out = '<item id="'
				+ itemNode.id
				+ '" '
				+ (this._getOpenState(itemNode) == 1 ? ' open="1" ' : '')
				+ (lid == itemNode.id ? ' select="1"' : '')
				+ ' text="'
				+ text
				+ '"'
				+ (((this.XMLsource) && (itemNode.XMLload == 0))
						? " child=\"1\" "
						: "") + '>';
	else
		out = '<item id="'
				...
				+ itemNode.images[2]
				+ '" '
				+ (itemNode.acolor ? ('aCol="' + itemNode.acolor + '" ') : '')
				+ (itemNode.scolor ? ('sCol="' + itemNode.scolor + '" ') : '')
				+ (itemNode.checkstate == 1
						? 'checked="1" '
						: (itemNode.checkstate == 2 ? 'checked="-1"' : ''))
				+ (itemNode.closeable ? 'closeable="1" ' : '') + '>';

	if ((this._xuserData) && (itemNode._userdatalist)) {
		var names = itemNode._userdatalist.split(",");
		for (var i = 0; i < names.length; i++)
			out += "<userdata name=\"" + names[i] + "\">" + this._apreUC
					+ itemNode.userData["t_" + names[i]] + this._apstUC
					+ "</userdata>";
	}
...
dhtmlXTreeObject.prototype.saveState = function(name, cookie_param) {
	...
	if (z.length > kusok) {
		...
		for (var i = 0; i < Math.ceil(z.length / kusok); i++) {
			this.setCookie("treeStatex" + name + "x" + i, z.substr(i * kusok,
							kusok), cookie_param);
		}
	} else
...
dhtmlXTreeObject.prototype.loadState = function(name) {
	...
	if (z.length) {
		...
		this
				.loadXMLString((this.utfesc == "utf8")
						? decodeURI(z)
						: unescape(z));
	}
...
dhtmlXTreeObject.prototype._createSelf = function() {
	...
	this.onRowSelect = function(e, htmlObject, mode) {
		...
		if (htmlObject.parentObject.treeNod.assMode)
			htmlObject.parentObject.treeNod
					.saveSelectedItem(htmlObject.parentObject.treeNod.assCookieName);
	}
...
dhtmlXTreeObject.prototype._openAllNodeChilds = function(itemNode) {
	...
	for (var i = 0; i < itemNode.childsCount; i++) {
		...
		if (itemNode.childNodes[i].childsCount > 0)
			this._openAllNodeChilds(itemNode.childNodes[i]);

		if ((itemNode.childNodes[i].XMLload == 0)
				|| (itemNode.childNodes[i].unParsed))
			this.ClosedElem.push(itemNode.childNodes[i]);
	}
}
...
dhtmlXTreeObject.prototype._loadAndOpen = function(that) {
	...
	if (that.ClosedElem.length > 0)
		that.G_node = that.ClosedElem.shift();
	...
	if (that.G_node)
		if (that.G_node.unParsed)
			...
			window.setTimeout(function() {
						that._loadDynXML(that.G_node.id);
					}, 100);
	else {
