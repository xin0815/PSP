		...
		if ((isreparse) && (this._edsbpsA)) {
			for (var j = 0; j < this._edsbpsA.length; j++)
				if (this._edsbpsA[j][2] == itemId) {
					dhtmlxError.throwError("getItem",
							"Requested item still in parsing process.", itemId);
					return null
				}
		};
		return z
		...
		if (!n)
			dhtmlxError
					.throwError(
							...
							"The item "
									+ itemId
									+ " not operable. Seems you have non-unique|incorrect IDs in tree's XML.",
							itemId)
	};
	return n
};
...
dhtmlXTreeObject.prototype._parseItem = function(c, temp, preNode, befNode) {
	...
	if (a.tooltip)
		if (this._dhxTT)
			dhtmlxTooltip.setTooltip(newNode.span.parentNode.parentNode,
					a.tooltip);
		else
	...
	if (this.parserExtension)
		this.parserExtension._parseExtension(node.childNodes[i],
				this.parserExtension, a.id, parentId);
...
dhtmlXTreeObject.prototype._parse = function(p, parentId, level, start) {
	...
	p.each("item", function(c, i) {
		...
		if ((this._epgps) && (this._epgpsC == this.npl)) {
			this._setNextPageSign(temp, this.npl + 1 * (start || 0), level,
					node);
			return -1
		};
...
dhtmlXTreeObject.prototype._redrawFrom = function(dhtmlObject, itemObject,
	...
	for (var i = (start ? start - 1 : 0); i < tempx.childsCount; i++) {
		...
		if (this.childCalc != null) {
			if ((tempx.childNodes[i].unParsed)
					|| ((!tempx.childNodes[i].XMLload) && (this.XMLsource))) {
				if (tempx.childNodes[i]._acc)
					tempx.childNodes[i].span.innerHTML = tempx.childNodes[i].label
							+ this.htmlcA
							+ tempx.childNodes[i]._acc
							+ this.htmlcB;
				else
					tempx.childNodes[i].span.innerHTML = tempx.childNodes[i].label
			};
			if ((tempx.childNodes[i].childNodes.length) && (this.childCalc)) {
				if (this.childCalc == 1) {
					tempx.childNodes[i].span.innerHTML = tempx.childNodes[i].label
							+ this.htmlcA
							+ tempx.childNodes[i].childsCount
							+ this.htmlcB
				};
				if (this.childCalc == 2) {
					var zCount = tempx.childNodes[i].childsCount
							- (tempx.childNodes[i].pureChilds || 0);
					if (zCount)
						tempx.childNodes[i].span.innerHTML = tempx.childNodes[i].label
								+ this.htmlcA + zCount + this.htmlcB;
					if (tempx.pureChilds)
						tempx.pureChilds++;
					else
						tempx.pureChilds = 1
...
dhtmlXTreeObject.prototype._selectItem = function(node, e) {
	...
	if ((node.i_sel) && (this._amsel) && (e) && (e.ctrlKey))
		this._unselectItem(node);
			...
			&& ((!this._amselS) || (this._selected.length == 0) || (this._selected[0].parentObject == node.parentObject)))
		if ((this._amsel)
				&& (e)
				&& (e.shiftKey)
				&& (this._selected.length != 0)
				&& (this._selected[this._selected.length - 1].parentObject == node.parentObject)) {
...
dhtmlXTreeObject.prototype._fixChildCountLabel = function(itemNode, index) {
	...
	if ((itemNode.unParsed) || ((!itemNode.XMLload) && (this.XMLsource))) {
		if (itemNode._acc)
			itemNode.span.innerHTML = itemNode.label + this.htmlcA
					+ itemNode._acc + this.htmlcB;
		else
	...
	switch (this.childCalc) {
		case 1 :
			if (itemNode.childsCount != 0)
				itemNode.span.innerHTML = itemNode.label + this.htmlcA
						+ itemNode.childsCount + this.htmlcB;
			else
				itemNode.span.innerHTML = itemNode.label;
			break;
		case 2 :
			...
			if (z != 0)
				itemNode.span.innerHTML = itemNode.label + this.htmlcA + z
						+ this.htmlcB;
			else
				itemNode.span.innerHTML = itemNode.label;
			break;
		case 3 :
			if (itemNode.childsCount != 0) {
				...
				itemNode.span.innerHTML = itemNode.label + this.htmlcA + bcc
						+ this.htmlcB;
				itemNode._acc = bcc
			} else {
				itemNode.span.innerHTML = itemNode.label;
			...
			;
			if ((itemNode.parentObject)
					&& (itemNode.parentObject != this.htmlNode))
...
dhtmlXTreeObject.prototype.setUserData = function(itemId, name, value) {
	...
	if (name == "hint")
		if (this._dhxTT)
			dhtmlxTooltip.setTooltip(
					sNode.htmlNode.childNodes[0].childNodes[0], value);
		else
...
dhtmlXTreeObject.prototype.setItemText = function(itemId, newLabel, newTooltip) {
	...
	if (this._dhxTT)
		dhtmlxTooltip.setTooltip(temp.span.parentNode.parentNode,
				(newTooltip || ""));
	else
...
dhtmlXTreeObject.prototype._setMove = function(htmlNode, x, y) {
	if (htmlNode.parentObject.span) {
		...
		if (this.dadmode == 2) {
			var z = y
					- a1
					+ this.allTree.scrollTop
					+ (document.body.scrollTop || document.documentElement.scrollTop)
					- 2 - htmlNode.offsetHeight / 2;
		...
		if (this.dadmodec == 0) {
			...
			this._clearMove();
			this.selectionBar.style.top = (a1
					- a2
					+ ((parseInt(htmlNode.parentObject.span.parentNode.previousSibling.childNodes[0].style.height) || 18) - 1) + this.dadmodefix)
					+ "px";
...
dhtmlXTreeObject.prototype._getPrevNode = function(node, mode) {
	if ((node.tr) && (node.tr.previousSibling)
			&& (node.tr.previousSibling.nodem))
...
dhtmlXTreeObject.prototype.findItem = function(searchStr, direction, top) {
	var z = this._findNodeByLabel(searchStr, direction, (top
					? this.htmlNode
					: null));
	if (z) {
...
dhtmlXTreeObject.prototype.findItemIdByLabel = function(searchStr, direction,
		top) {
	var z = this._findNodeByLabel(searchStr, direction, (top
					? this.htmlNode
					: null));
...
dhtmlXTreeObject.prototype._findNodeByLabel = function(searchStr, direction,
		fromNode) {
	var searchStr = searchStr.replace(new RegExp("^( )+"), "").replace(
			new RegExp("( )+$"), "");
	searchStr = new RegExp(searchStr.replace(/([\*\+\\\[\]\(\)]{1})/gi, "\\$1")
					.replace(/ /gi, ".*"), "gi");
	if (!fromNode) {
	...
	if (!direction) {
		if ((fromNode.unParsed)
				&& (this.findStrInXML(fromNode.unParsed.d, "text", searchStr)))
...
dhtmlXTreeObject.prototype._checkMSelectionLogic = function() {
	...
	for (var i = 0; i < this._selected.length; i++)
		for (var j = 0; j < this._selected.length; j++)
			if ((i != j)
					&& (this._checkPNodes(this._selected[j], this._selected[i])))
...
dhtmlXTreeObject.prototype.enableDistributedParsing = function(mode, count,
		delay) {
...
dhtmlXTreeObject.prototype._distributedStart = function(node, start, parentId,
		level, start2) {
	if (!this._edsbpsA)
		this._edsbpsA = new Array();
...
dhtmlXTreeObject.prototype._setPrevPageSign = function(node, pos, level,
		xmlnode) {
	var z = document.createElement("DIV");
...
dhtmlXTreeObject.prototype._setNextPageSign = function(node, pos, level,
		xmlnode) {
	var z = document.createElement("DIV");
	...
	w.span.parentNode.parentNode.parentNode.parentNode.parentNode
			.appendChild(z)
};
dhtmlXTreeObject.prototype._nextPageCall = function(node) {
	...
	this._parseXMLTree(this, node._pageData[3], node._pageData[0].id,
			node._pageData[2], null, node._pageData[1]);
...
dhtmlXTreeObject.prototype.refreshItems = function(itemIdList, source) {
	...
	this.loadXML((source || this.XMLsource)
			+ getUrlSymbol(source || this.XMLsource) + "ids="
			+ this._escape(itemIdList))
};
dhtmlXTreeObject.prototype.updateItem = function(itemId, name, im0, im1, im2,
		achecked) {
	var sNode = this._globalIdStorageFind(itemId);
	sNode.userData = new cObject();
	if (name)
		sNode.label = name;
	sNode.images = new Array(im0 || this.imageArray[0], im1
					|| this.imageArray[1], im2 || this.imageArray[2]);
...
dhtmlXTreeObject.prototype.setItemTopOffset = function(itemId, value) {
	...
	for (var i = 0; i < z.childNodes.length; i++) {
		if (i != 0)
			...
			if ((node.parentObject.id != this.rootId || node.parentObject.childNodes[0] != node)
					&& this.treeLinesOn)
				w.style.backgroundImage = "url(" + this.imPath
						+ this.lineArray[5] + ")";
			w.innerHTML = "&nbsp;";
...
dhtmlXTreeObject.prototype.showItemCheckbox = function(itemId, state) {
	...
	itemId.span.parentNode.previousSibling.previousSibling.childNodes[0].style.display = (!itemId.nocheckbox)
			? ""
			: "none"
};
